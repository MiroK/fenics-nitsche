'''
    ...
    transient Navier-Stokes problem. The scheme is compared with IPCS and mixed
    solvers. Single-phase.
'''
...
def mixed_solver(mesh_):
    ...
    F = (rho/dt)*inner(u - u0, v)*dx + inner(dot(grad(u), u), v)*dx\
        - inner(p, div(v))*dx - inner(q, div(u))*dx\
        + inner(2*mu*D(U), D(v))*dx - inner(rho*f, v)*dx
    ...
    comm = mesh.mpi_comm()
    u_file = XDMFFile(comm, 'results/u_stokes_mixed_{0}.xdmf'.format(mesh_))
    p_file = XDMFFile(comm, 'results/p_stokes_mixed_{0}.xdmf'.format(mesh_))
...
def ipcs_solver(mesh_):
    ...
    nonlinearity = inner(dot(grad(U_cn), U_ab), v)*dx

    F0 = (rho/dt)*inner(u - u0, v)*dx + nonlinearity\
        + mu*inner(grad(U_cn), grad(v))*dx + inner(grad(p0), v)*dx\
        - inner(rho*f, v)*dx
    a0, L0 = system(F0)
    ...
    comm = mesh.mpi_comm()
    u_file = XDMFFile(comm, 'results/u_stokes_ipcs_{0}.xdmf'.format(mesh_))
    p_file = XDMFFile(comm, 'results/p_stokes_ipcs_{0}.xdmf'.format(mesh_))
    u_file.parameters['rewrite_function_mesh'] = False
    p_file.parameters['rewrite_function_mesh'] = False
...
def yanenko_solver(mesh_):
    ...
    comm = mesh.mpi_comm()
    u_file = XDMFFile(comm, 'results/u_stokes_yank_{0}.xdmf'.format(mesh_))
    p_file = XDMFFile(comm, 'results/p_stokes_yank_{0}.xdmf'.format(mesh_))
    u_file.parameters['rewrite_function_mesh'] = False
    p_file.parameters['rewrite_function_mesh'] = False
